...
let day = "Sunday",
    time1 = 16;
let movie =
    day === "Sunday" && time1 >= 10 && time1 <= 14 ? "Running" : "Not Running";
console.log(movie);
...
let numb1 = numb.filter((val) => {
    //Function inside array.filter()
    console.log(val);
    if (val < 20) {
        return true;
    }
...
console.log(emailRef); //Just to check
...
async function func2() {
    ...
    try {
        ...
        console.log(e);
        document.querySelector('.video').innerHTML = e;
    }
    document.querySelector('.video').innerHTML = await getVideo(status);
}
func2();
